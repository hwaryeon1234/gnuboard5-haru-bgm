...
// 수정 요청인 경우, 이 음악이 정말 내 것인지 먼저 확인합니다.
// (다른 사람 mf_id를 넣어 보내는 것을 막기 위한 소유권 검증입니다.)
if ($mf_id) {
    $owner_check = sql_fetch("SELECT mf_id FROM `{$music}` WHERE mf_id='{$mf_id}' AND mb_id='{$mb_id}'");
    if (!$owner_check) alert('내가 등록한 음악만 수정할 수 있습니다.', HB_URL.'/my_music_list.php');
}
...
if ($source === 'youtube') {
    ...
    if ($mf_id) {
        ...
        $old = sql_fetch("SELECT * FROM `{$music}` WHERE mf_id='{$mf_id}' AND mb_id='{$mb_id}'");
        hb_delete_old_file_if_needed($old);
        sql_query("UPDATE `{$music}` SET mf_title='{$title_sql}', mf_source='youtube', mf_file='', mf_org_name='', mf_mime='', mf_size='0', mf_youtube_url='{$url_sql}', mf_youtube_id='{$yt_sql}', mf_volume='{$volume}', mf_type='{$type_sql}', mf_memo='{$memo_sql}', mf_use='{$use}', mf_updated_at=NOW() WHERE mf_id='{$mf_id}' AND mb_id='{$mb_id}'");
        alert('저장되었습니다.', HB_URL.'/my_music_list.php');
    }

    $bulk = $url."\n".(isset($_POST['bulk_youtube_urls']) ? $_POST['bulk_youtube_urls'] : '');
    $ids = hb_create_youtube_musics_from_text($bulk, $title, $volume, $type, $memo !== '' ? $memo : 'YouTube 등록', $member['mb_id']);
    if (!$ids) alert('YouTube 링크를 1개 이상 입력해주세요. 일반 영상 URL, youtu.be 링크, 영상 ID를 사용할 수 있습니다.');
    alert(count($ids).'개 YouTube 음악을 등록했습니다.', HB_URL.'/my_music_list.php');
}
...
if ($mf_id) {
    ...
    if (!$title) {
        $old = sql_fetch("SELECT mf_title FROM `{$music}` WHERE mf_id='{$mf_id}' AND mb_id='{$mb_id}'");
        $title = $old && $old['mf_title'] ? $old['mf_title'] : '내 BGM 음악';
    }
    $title_sql = hb_escape($title);
    $type_sql = hb_escape($type);
    $memo_sql = hb_escape($memo);
    sql_query("UPDATE `{$music}` SET mf_title='{$title_sql}', mf_source='file', mf_youtube_url='', mf_youtube_id='', mf_volume='{$volume}', mf_type='{$type_sql}', mf_memo='{$memo_sql}', mf_use='{$use}', mf_updated_at=NOW() {$file_sql} WHERE mf_id='{$mf_id}' AND mb_id='{$mb_id}'");
    alert('저장되었습니다.', HB_URL.'/my_music_list.php');
}
...
$msg = $ok.'개 음악을 등록했습니다.';
if ($fail) $msg .= '\n실패: '.count($fail).'개';
alert($msg, HB_URL.'/my_music_list.php');
?>
