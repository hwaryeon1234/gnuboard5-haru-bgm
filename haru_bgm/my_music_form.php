...
$g5['title'] = '내 음악 등록';
$mb_id_raw = isset($member['mb_id']) ? $member['mb_id'] : '';
...
if ($mf_id) {
    ...
    $mb_id_sql = hb_escape($mb_id_raw);
    $found = sql_fetch("SELECT * FROM `{$music}` WHERE mf_id='{$mf_id}' AND mb_id='{$mb_id_sql}'");
    if (!$found) alert('내가 등록한 음악만 수정할 수 있습니다.', HB_URL.'/my_music_list.php');
...
<div class="hb-wrap hb-radio">
    <section class="hb-page-head"><div><p class="hb-kicker">MY MUSIC</p><h1><?php echo $mf_id ? '내 음악 수정' : '내 음악 업로드 / YouTube 등록'; ?></h1><p>파일은 드래그앤드롭으로 여러 개, YouTube 링크도 여러 줄로 한 번에 등록할 수 있습니다. 등록한 음악은 나만 보고 사용할 수 있어요.</p></div><a class="hb-btn" href="<?php echo HB_URL; ?>/my_music_list.php">목록으로</a></section>
    <form class="hb-card hb-form" method="post" action="<?php echo HB_URL; ?>/my_music_update.php" enctype="multipart/form-data">
        ...
        <label>음악 제목 <span class="hb-muted-mini">여러 파일/링크 등록 시 비워두면 파일명 또는 영상 ID 기준으로 자동 등록됩니다.</span><input type="text" name="mf_title" value="<?php echo hb_e($row['mf_title']); ?>" placeholder="예: 저녁 산책 음악 / 잠들기 전 음악"></label>
