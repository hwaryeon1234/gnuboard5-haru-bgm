...
$rows = 50;
$from = ($page - 1) * $rows;
$total = sql_fetch("SELECT COUNT(*) AS cnt FROM `{$music}` WHERE mb_id='{$mb_id}'");
$total_count = (int)$total['cnt'];
$total_page = max(1, (int)ceil($total_count / $rows));
$res = sql_query("SELECT * FROM `{$music}` WHERE mb_id='{$mb_id}' ORDER BY mf_id DESC LIMIT {$from}, {$rows}");
?>
...
<div class="hb-wrap hb-radio">
    <section class="hb-page-head">
        <div>
            <p class="hb-kicker">MY MUSIC</p>
            <h1>내 음악 보관함</h1>
            <p>내가 등록한 음악만 보입니다. 내 시간표/내 시간대에서 선택할 수 있어요.</p>
        </div>
        <div class="hb-actions">
            <a class="hb-btn" href="<?php echo HB_URL; ?>/player.php">하루브금 홈</a>
            <a class="hb-btn hb-btn-primary" href="<?php echo HB_URL; ?>/my_music_form.php">+ 음악 등록</a>
        </div>
    </section>
    <section class="hb-card">
        ...
        <div class="hb-table-wrap">
            <table class="hb-table"><thead><tr><th>ID</th><th>제목</th><th>종류</th><th>파일/링크</th><th>볼륨</th><th>상태</th><th>미리듣기</th><th>관리</th></tr></thead><tbody>
            ...
            ?>
                <tr>
                    ...
                    <td class="hb-row-actions"><a class="hb-btn hb-btn-small" href="<?php echo HB_URL; ?>/my_music_form.php?mf_id=<?php echo $mf_id; ?>">수정</a><a class="hb-btn hb-btn-small hb-danger" href="<?php echo HB_URL; ?>/my_music_delete.php?mf_id=<?php echo $mf_id; ?>" onclick="return confirm('삭제할까요? 연결된 내 시간표/시간대에서도 함께 정리됩니다.');">삭제</a></td>
                </tr>
            <?php } if ($i === 0) { ?><tr><td colspan="8"><div class="hb-empty"><div class="hb-empty-icon">🎵</div><strong>등록한 음악이 없어요</strong><p>파일을 올리거나 YouTube 링크를 등록하면 내 시간표에서 바로 선택할 수 있어요.</p><a class="hb-btn hb-btn-small hb-btn-primary" href="<?php echo HB_URL; ?>/my_music_form.php">+ 음악 등록</a></div></td></tr><?php } ?>
            </tbody></table>
        </div>
        <?php if ($total_page > 1) { ?><div class="hb-actions hb-pager"><?php if ($page > 1) { ?><a class="hb-btn" href="?page=<?php echo $page-1; ?>">이전</a><?php } ?><span class="hb-muted">50개씩 표시 중</span><?php if ($page < $total_page) { ?><a class="hb-btn" href="?page=<?php echo $page+1; ?>">다음</a><?php } ?></div><?php } ?>
    </section>
</div>
