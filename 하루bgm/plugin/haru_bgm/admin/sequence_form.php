...
<form class="hb-card hb-form" method="post" action="<?php echo HB_URL; ?>/admin/sequence_update.php"><?php echo hb_csrf_field(); ?>
<input type="hidden" name="seq_id" value="<?php echo (int)$row['seq_id']; ?>">
<div class="hb-two"><label>순서표 이름<input type="text" name="seq_title" maxlength="255" value="<?php echo hb_e($row['seq_title']); ?>" required placeholder="예: 주일 오전예배"></label><label>구분<select name="seq_type"><option value="church" <?php echo $row['seq_type']==='church'?'selected':''; ?>>교회</option><option value="broadcast" <?php echo $row['seq_type']==='broadcast'?'selected':''; ?>>방송</option><option value="event" <?php echo $row['seq_type']==='event'?'selected':''; ?>>행사</option><option value="store" <?php echo $row['seq_type']==='store'?'selected':''; ?>>매장/학교</option><option value="general" <?php echo $row['seq_type']==='general'?'selected':''; ?>>기타</option></select></label></div>
<label>메모<textarea name="seq_memo" rows="3" maxlength="<?php echo HB_MAX_MEMO_CHARS; ?>" placeholder="예: 예배 전 묵상 → 입례송 → 찬양 → 헌금송 → 폐회송"><?php echo hb_e($row['seq_memo']); ?></textarea></label>
<div class="hb-source-box"><strong>순서 항목</strong><p class="hb-muted-mini">위에서부터 순서대로 진행됩니다. 순서명은 운영판에 크게 표시됩니다.</p><div class="hb-track-list"><?php echo hb_get_sequence_music_select_rows($ids, $titles, $memos, HB_MAX_SEQUENCE_ITEMS); ?></div></div>
<div class="hb-source-box"><strong>YouTube 링크 추가 등록</strong><p class="hb-muted-mini">여러 줄로 붙여넣으면 음악 보관함에 등록되고 순서표 뒤에 자동 추가됩니다.</p><label>YouTube 링크/영상 ID<textarea name="quick_youtube_urls" rows="5" placeholder="https://youtu.be/...\nhttps://www.youtube.com/watch?v=..."></textarea></label><label>기본 제목<input type="text" name="quick_youtube_title" maxlength="255" placeholder="비우면 순서표 이름으로 등록"></label></div>
