...
$g5['title'] = '공통 시간대 등록';
$bl_id = isset($_GET['bl_id']) ? hb_int_value($_GET['bl_id'], 0) : 0;
...
if ($bl_id) {
    ...
    $found = sql_fetch("SELECT * FROM `{$block}` WHERE bl_id='{$bl_id}' AND bl_scope='global'", false);
...
<div id="haruBgmAdminApp" class="hb-app"><?php echo hb_nav_admin(); ?><main class="hb-app-main"><div class="hb-wrap">
    ...
    <form class="hb-card hb-form" method="post" action="<?php echo HB_URL; ?>/admin/block_update.php"><?php echo hb_csrf_field(); ?>
        <input type="hidden" name="bl_id" value="<?php echo (int)$row['bl_id']; ?>">
        <label>시간대 제목<input type="text" name="bl_title" maxlength="255" value="<?php echo hb_e($row['bl_title']); ?>" placeholder="예: 예배 전 묵상 BGM / 오전 매장 음악" required></label>
        ...
        <div class="hb-fieldset"><strong>음악 목록</strong><p class="hb-muted">위에서 아래 순서대로 재생됩니다. 처음에는 4칸만 보이고, + 음악 추가로 최대 100곡까지 늘릴 수 있습니다.</p><div class="hb-track-list"><?php echo hb_get_block_music_select_rows($selected_ids, HB_MAX_BLOCK_ITEMS); ?></div></div>
...
https://www.youtube.com/watch?v=..."></textarea>
            ...
            <label>YouTube 제목 접두어
                <input type="text" name="quick_youtube_title" maxlength="255" value="" placeholder="비우면 시간대 제목으로 자동 등록">
            </label>
