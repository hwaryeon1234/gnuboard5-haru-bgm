...
if (!defined('HB_PLUGIN_VERSION')) define('HB_PLUGIN_VERSION', '1.5.46');
if (!defined('HB_ASSET_VERSION')) define('HB_ASSET_VERSION', '20260904-v1546-r1');
...
function hb_user_has_admin_auth($menu_id='', $attr='r') {
    ...
    // 1.5.17 이하에서 부여된 390번대 권한을 즉시 잃지 않도록 읽기 호환을 유지합니다.
    if ($auth === '' && preg_match('/^990(1[0-9]0)$/', $menu_id, $m)) {
        $legacy_menu_id = '390'.$m[1];
        $legacy_sql = hb_escape($legacy_menu_id);
        $legacy_row = sql_fetch("SELECT au_auth FROM `{$g5['auth_table']}` WHERE mb_id='{$mb_sql}' AND au_menu='{$legacy_sql}' LIMIT 1", false);
        $auth = $legacy_row && isset($legacy_row['au_auth']) ? strtolower((string)$legacy_row['au_auth']) : '';
    }
...
function hb_is_plugin_admin() {
    ...
    // 1.5.17 이하의 390번대 권한도 플러그인 관리자 판정에 포함합니다.
    foreach (array('390100','390110','390120','390130','390140','390150','390160','390180','390190') as $legacy_menu_id) {
        $menu_ids[] = $legacy_menu_id;
    }
