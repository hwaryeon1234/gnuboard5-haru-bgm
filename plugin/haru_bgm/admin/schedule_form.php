...
$g5['title'] = '공통 시간표 등록';
$sc_id = isset($_GET['sc_id']) ? hb_int_value($_GET['sc_id'], 0) : 0;
$row = array('sc_id'=>0, 'mf_id'=>0, 'sc_title'=>'', 'sc_time'=>'09:00:00', 'sc_play_mode'=>'once', 'sc_end_time'=>'10:00:00', 'sc_repeat'=>0, 'sc_days'=>'0,1,2,3,4,5,6', 'sc_start_date'=>'', 'sc_end_date'=>'', 'sc_use'=>1, 'sc_sort'=>0);
if ($sc_id) {
    $schedule = hb_table('schedule');
    $found = sql_fetch("SELECT * FROM `{$schedule}` WHERE sc_id='{$sc_id}' AND sc_scope='global'", false);
...
<div id="haruBgmAdminApp" class="hb-app"><?php echo hb_nav_admin(); ?><main class="hb-app-main"><div class="hb-wrap">
    ...
    <form class="hb-card hb-form" method="post" action="<?php echo HB_URL; ?>/admin/schedule_update.php"><?php echo hb_csrf_field(); ?>
        <input type="hidden" name="sc_id" value="<?php echo (int)$row['sc_id']; ?>">
        <label>제목<input type="text" name="sc_title" maxlength="255" value="<?php echo hb_e($row['sc_title']); ?>" placeholder="예: 점심시간 알림 / 예배 전 묵상 BGM" required></label>
        ...
        <div class="hb-source-box hb-range-options" id="hbRangeOptions">
            ...
            <p class="hb-muted-mini">예: 09:30~10:00 예배 전 묵상곡, 13:00~14:00 방송 대기음처럼 시작~종료 시간 안에서만 재생됩니다.</p>
            ...
            <p class="hb-youtube-note">여러 곡을 시간 안에 순서대로/랜덤으로 길게 틀려면 <?php if ($can_block_write) { ?><a href="<?php echo HB_URL; ?>/admin/block_form.php">공통 시간대 묶음</a><?php } else { ?>공통 시간대 묶음<?php } ?>도 함께 사용할 수 있습니다.</p>
        </div>
        <label>첫 번째(대표) 음악
            <select name="mf_id">
                <option value="">음악 선택</option><?php echo hb_get_music_options((int)$row['mf_id']); ?>
            </select>
            <span class="hb-muted-mini">첫 재생 순서입니다. 아래 추가 음악/YouTube와 함께 같이 저장할 수 있습니다.</span>
        </label>
        <div class="hb-source-box">
            <strong>추가 음악 함께 넣기</strong>
            <p class="hb-muted-mini">업로드한 파일 음악과 등록된 YouTube 음악을 같이 섞을 수 있습니다. 위 대표 음악 + 아래 목록이 한 세트로 저장됩니다.</p>
            <div class="hb-track-list"><?php echo hb_get_schedule_music_select_rows($extra_ids, HB_MAX_SCHEDULE_ITEMS - 1); ?></div>
        </div>
        <div class="hb-source-box">
            <strong>YouTube 링크 추가 등록</strong>
            <p class="hb-muted-mini">링크를 입력하면 음악 보관함에 자동 등록되고, 이 시간표 세트에 함께 추가됩니다. 이제 기존 파일 음악과 함께 같이 쓸 수 있습니다.</p>
...
https://www.youtube.com/watch?v=..."></textarea>
            ...
            <label>YouTube 음악 제목
                <input type="text" name="quick_youtube_title" maxlength="255" value="" placeholder="비우면 시간표 제목으로 자동 등록">
            </label>
            ...
            <p class="hb-youtube-note">파일 음악과 YouTube를 동시에 쓰고 싶다면 대표 음악/추가 음악을 고른 뒤 여기에 링크를 더 넣으면 됩니다.</p>
        </div>
        <div class="hb-fieldset"><strong>요일</strong><div class="hb-checks"><?php foreach (hb_days_all() as $d => $label) { ?><label><input type="checkbox" name="sc_days[]" value="<?php echo $d; ?>" <?php echo in_array((string)$d, $days_selected, true) ? 'checked' : ''; ?>> <?php echo $label; ?></label><?php } ?></div></div>
        <div class="hb-two"><label>시작일 선택<input type="date" name="sc_start_date" value="<?php echo hb_e($row['sc_start_date']); ?>"></label><label>종료일 선택<input type="date" name="sc_end_date" value="<?php echo hb_e($row['sc_end_date']); ?>"></label></div>
        <label>정렬값<input type="number" name="sc_sort" value="<?php echo (int)$row['sc_sort']; ?>"></label>
