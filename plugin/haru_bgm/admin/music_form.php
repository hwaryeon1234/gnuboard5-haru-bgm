...
$g5['title'] = '하루브금 음악 등록';
$mf_id = isset($_GET['mf_id']) ? hb_int_value($_GET['mf_id'], 0) : 0;
...
if ($mf_id) {
    ...
    $found = sql_fetch("SELECT * FROM `{$music}` WHERE mf_id='{$mf_id}'", false);
...
<div id="haruBgmAdminApp" class="hb-app"><?php echo hb_nav_admin(); ?><main class="hb-app-main"><div class="hb-wrap">
    ...
    <form class="hb-card hb-form" method="post" action="<?php echo HB_URL; ?>/admin/music_update.php" enctype="multipart/form-data"><?php echo hb_csrf_field(); ?>
        ...
        <label>음악 제목 <span class="hb-muted-mini">여러 파일/링크 등록 시 비워두면 파일명 또는 영상 ID 기준으로 자동 등록됩니다.</span><input type="text" name="mf_title" maxlength="255" value="<?php echo hb_e($row['mf_title']); ?>" placeholder="예: 밤 작업용 재즈 / 예배 전 묵상 BGM"></label>
...
영상 ID만 넣어도 됩니다."></textarea>
        ...
        <label>메모<textarea name="mf_memo" rows="4" maxlength="<?php echo HB_MAX_MEMO_CHARS; ?>"><?php echo hb_e($row['mf_memo']); ?></textarea></label>
